...
const Navbar = () => {
  ...
  //create a new toggleDrawer state 
  const [toggleDrawer, setToggleDrawer] = useState(false);
  //we are hard coding that address here that is used in connect btn
    ...
    <div className='flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6'>
      ...
      {/* For Connect Button */}
        <div className="sm:flex hidden flex-row justify-end gap-4">
          <CustomButton 
            btnType="button"
            title={address ? "Create a campaign" : "Connect"} //It is going to depend if we have an active address
        ...
        <div className="sm:hidden flex justify-between items-center relative">
          ...
          <img 
            ...
            onClick={() => setToggleDrawer((prev) => !prev)} // Not a good practice to change state like : (!toggleDrawer) rather than use (prev) => !prev
          />
          <div className={`absolute top-[60px] right-0 left-0
          bg-[#1c1c24] z-10 shadow-secondary py-4 ${ !toggleDrawer  ? '-translate-y-[100vh]'
          : 'translate-y-[0]'} transition-all duration-700 `}> {/*Note : ${ !toggleDrawer  ? '-translate-y-[100vh]'
          : 'translate-y-[0]'} */}
            <ul className='mb-4 '>
                {navlinks.map((Link) => (
                  <li
